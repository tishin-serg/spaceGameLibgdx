...
public class Hero {
    ...
    public void render(SpriteBatch spriteBatch) {
        spriteBatch.draw(texture, position.x - 32, position.y - 32, 32, 32, 64, 64, 1, 1, angle, 0, 0, 64, 64, false, false);
    }
    ...
    public void update(float dt) {
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.W)) {
            ...
            lastDisplacement.set(MathUtils.cosDeg(angle) * 240.0f, MathUtils.sinDeg(angle) * 240.0f);
        } else if (Gdx.input.isKeyPressed(Input.Keys.S)) {
            position.x += MathUtils.cosDeg(angle) * (-120.0f) * dt;
            position.y += MathUtils.sinDeg(angle) * (-120.0f) * dt;
            lastDisplacement.set(MathUtils.cosDeg(angle) * (-120.0f), MathUtils.sinDeg(angle) * (-120.0f));
        } else {
